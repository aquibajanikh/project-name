  ...
  const BookCollection = {
    ...
    removeBookById: function(id) {
      ...
      if (bookIndex === -1) {
        ...
        console.log(`"${removedBook[0].title}" has been removed from the collection.`);
    ...
    searchByTitle: function(title) {
      ...
      const matchedTitles = this.books.filter(book => book.title.includes(title));
      ...
      if (matchedTitles.length === 0) {
        ...
        matchedTitles.forEach(book => console.log(`- ${book.title} by ${book.author} (${book.year})`));
    ...
    computeAverageRating: function() {
      ...
      const totalRatings = this.books.reduce((acc, book) => acc + book.ratings.length, 0);
 
      const totalScore = this.books.reduce((acc, book) => acc + book.ratings.reduce((acc, rating) => acc + rating, 0), 0);
      ...
      if (totalRatings === 0) {
        ...
        console.log(`The average rating of books in the collection is ${(totalScore / totalRatings).toFixed(1)}.`);
