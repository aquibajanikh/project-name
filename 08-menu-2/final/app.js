...
function diplayMenuItems(menuItems) {
  ...
  const addToCartButtons = sectionCenter.querySelectorAll(".add-to-cart");
...
function addToCart(id) {
  const selectedMenuItem = menu.find(function (item) {
    return item.id === id;
  });
  ...
  if (existingItem) {
    ...
    const cartItem = { ...selectedMenuItem, amount: 1 };
    cart.push(cartItem);
  }
...
function increaseAmount(id) {
  const selectedCartItem = cart.find(function (item) {
    return item.id === id;
  });
...
function updateCart() {
  ...
  cart.forEach(function (item) {
    ...
    cartItemsTotal += amount;
    cartTotalPrice += price * amount;
  });

  cartTotal.innerHTML = `$${cartTotalPrice.toFixed(2)}`;
  cartBtn.querySelector(".cart-items").textContent = cartItemsTotal;
}

// Add event listeners to dynamically added cart items
